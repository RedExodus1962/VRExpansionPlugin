...
#include "DrawDebugHelpers.h"
     ...
     // Degrees added to the target pitch per shot
// ---------------------------------------------------------
    ...
    Super(ObjectInitializer)
{
    ...
    // --- Initialize Additional Recoil Variables ---
    AdditionalRecoilPitch = 0.0f;
    AdditionalRecoilPitchTarget = 0.0f;
    AdditionalRecoilLerpSpeed = 5.0f;      // adjust as desired
    AdditionalRecoilDecaySpeed = 2.0f;     // adjust as desired
    AdditionalRecoilPerShot = 1.0f;        // adjust as desired (in degrees)
}
...
) 
{
    ...
    {
        BackEndRecoilStorage.Blend(BackEndRecoilStorage, BackEndRecoilTarget, FMath::Clamp(RecoilLerpRate * DeltaTime, 0.f, 1.f));
        BackEndRecoilTarget.Blend(BackEndRecoilTarget, FTransform::Identity, FMath::Clamp(RecoilDecayRate * DeltaTime, 0.f, 1.f));
    ...
    if (bHasActiveRecoil)
    {
        ...
        // Adjust the pivot of the recoil rotation by the PivotOffset vector
    ...
    // Check the grip lerp state, this is outside of the secondary attach check below because it can change the result of it
    ...
    if ((Grip.SecondaryGripInfo.bHasSecondaryAttachment && Grip.SecondaryGripInfo.SecondaryAttachment) || Grip.SecondaryGripInfo.GripLerpState == EGripLerpState::EndLerp)
    {
        ...
        if (SecondaryType != ESecondaryGripType::SG_Custom)
        {
            ...
            if (Grip.SecondaryGripInfo.GripLerpState == EGripLerpState::EndLerp)
            {
                ...
                // --- Apply Additional Pitch Recoil before early return ---
                AdditionalRecoilPitch = FMath::FInterpTo(AdditionalRecoilPitch, AdditionalRecoilPitchTarget, DeltaTime, AdditionalRecoilLerpSpeed);
                AdditionalRecoilPitchTarget = FMath::FInterpTo(AdditionalRecoilPitchTarget, 0.0f, DeltaTime, AdditionalRecoilDecaySpeed);
                {
                    FRotator AdditionalRot(AdditionalRecoilPitch, 0.0f, 0.0f);
                    FTransform AdditionalTransform(AdditionalRot);
                    WorldTransform = AdditionalTransform * WorldTransform;
                }
                return true;
            }
    ...
    // --- Apply Additional Pitch Recoil Layer (affecting only pitch) ---
    AdditionalRecoilPitch = FMath::FInterpTo(AdditionalRecoilPitch, AdditionalRecoilPitchTarget, DeltaTime, AdditionalRecoilLerpSpeed);
    AdditionalRecoilPitchTarget = FMath::FInterpTo(AdditionalRecoilPitchTarget, 0.0f, DeltaTime, AdditionalRecoilDecaySpeed);
    {
        FRotator AdditionalRot(AdditionalRecoilPitch, 0.0f, 0.0f);
...
void UGS_GunTools::AddRecoilInstance(const FTransform & RecoilAddition, FVector Optional_Location)
{
    ...
    else
    {
        ...
        // --- Increment Additional (Cumulative) Pitch Recoil Target ---
        AdditionalRecoilPitchTarget += AdditionalRecoilPerShot;
    }
}
